...
**Maze.java** - 26/6/16 미로 탐색 (백트래킹)
| 항목 | 설명 |
|---|---|
| 목적 | (0,0)에서 (N-1,N-1)까지의 경로를 재귀로 탐색 |
| 시간복잡도 | O(4^(N²)) — 최악의 경우, 실제로는 가지치기로 훨씬 적음 |
| 핵심 로직 | PATH_COLOR(3)로 마킹 후 4방향 탐색, 실패 시 BLOCKED_COLOR(2)로 백트래킹 |
...
**PowerSet.java** - 26/6/22 멱집합 (부분집합) 생성
| 항목 | 설명 |
|---|---|
| 목적 | 주어진 집합의 모든 부분집합을 재귀로 출력 |
| 시간복잡도 | O(2^n) — 각 원소를 포함/불포함 두 가지로 분기 |
| 핵심 로직 | include[] 배열로 각 원소의 포함 여부를 결정, k==n이면 출력 |
